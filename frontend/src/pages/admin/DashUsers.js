...
const DashUsers = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            width: 150,
            renderCell: (params) => (
                params.row.role === 1 ? "Admin" : params.row.role === 2 ? "Recruiter" : "Regular user"
            )
        },
        {
            field: 'createdAt',
            ...
            renderCell: (values) => (
                <Box sx={{ display: "flex", justifyContent: "space-between", width: "220px" }}>
                    <Button variant="contained" color="warning">
                        <Link style={{ color: "white", textDecoration: "none" }} to={`/admin/view/user/${values.row._id}`}>View Details</Link>
                    </Button>
